...
async fn fetch_trust(client: &reqwest::Client, pkg: &PackageRef) -> TrustResult {
    ...
    let (critical, high, medium, low) = classify_advisories(client, &body).await;
    tracing::debug!(
        package = %pkg.name,
    ...
    let is_stale = body
        ...
        .is_some_and(|ts| {
            ...
            chrono::DateTime::parse_from_rfc3339(ts)
                .is_ok_and(|dt| now.signed_duration_since(dt) > chrono::Duration::days(730))
        });
...
async fn classify_advisories(
    ...
    for id in &advisory_ids {
        ...
        let url = format!("https://api.deps.dev/v3/advisories/{}", url_encode(id));
        set.spawn(async move {
            ...
            val.get("cvss3Score").and_then(serde_json::Value::as_f64)
        });
    }
