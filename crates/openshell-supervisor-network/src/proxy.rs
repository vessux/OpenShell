...
async fn handle_transparent_tcp_connection(
    ...
    // http_context() does not know about the fork's cred_inject/echo/trust
    // fields (upstream-authored helper); populate them here so this transport
    // gets the same per-binary-scoped credential filtering, strip-and-replace
    // rewrite, and trust-check enforcement as the CONNECT/forward-HTTP paths.
    // secret_resolver is already None on this path (no per-request resolver
    // is threaded through transparent TCP), so filter_resolver_by_policy is a
    // no-op here, but is still called for consistency/fail-closed-by-default.
    ctx.secret_resolver = filter_resolver_by_policy(&opa_engine, &decision, &host, port, &None);
    ctx.allowed_secrets = query_allowed_secrets_for_policy(&opa_engine, &decision, &host, port);
    ctx.cred_inject = query_cred_inject_config(&opa_engine, &decision, &host, port);
    ctx.trust_check = query_trust_check_config(&opa_engine, &decision, &host, port);
...
async fn handle_tcp_connection(
    ...
    let l7_route = query_l7_route_snapshot(&decision, &host_lc, port);
    ...
    if is_echo {
        ...
        let mut ctx = crate::l7::relay::L7EvalContext {
            ...
            allowed_secrets: query_allowed_secrets_for_policy(
                &opa_engine,
                &decision,
                &host_lc,
                port,
        ...
        if let Some(route) = l7_route.as_ref().filter(|route| !route.configs.is_empty()) {
            let tunnel_engine = match opa_engine.clone_engine_for_tunnel(route.l7_policy_generation)
            {
                Ok(engine) => engine,
                Err(e) => {
    ...
    // Note: allowed_ips resolution (including the implicit-IP-host fallback)
    // now happens inside `hydrate_destination_plan` above -- it already
    // queries `query_allowed_ips` and builds the validation plan from it, so
    // there is no separate allowlist computation to do here anymore.
    ...
    // http_context() does not know about the fork's cred_inject/echo/trust
    // fields; populate them here from the same generation-pinned `decision`
    // so credential resolution stays per-binary-scoped and cred_inject/trust
    // enforcement runs on the CONNECT/forward-HTTP path exactly as before
    // this rebase. Never let a request reach the L7 proxy with an
    // unfiltered resolver -- filter_resolver_by_policy fails closed
    // (returns None) when the calling binary isn't in `allowed_secrets`.
    ctx.secret_resolver =
        filter_resolver_by_policy(&opa_engine, &decision, &host_lc, port, &secret_resolver);
    ctx.allowed_secrets = query_allowed_secrets_for_policy(&opa_engine, &decision, &host_lc, port);
    ctx.cred_inject = query_cred_inject_config(&opa_engine, &decision, &host_lc, port);
    ctx.echo = l7_route
        .and_then(|route| route.configs.first())
        .is_some_and(|snapshot| snapshot.config.echo);
    ctx.trust_cache = Some(trust_cache.clone());
    ctx.trust_check = query_trust_check_config(&opa_engine, &decision, &host_lc, port);
...
/// Fork: query the per-binary `allowed_secrets` list policy establishes for
/// this (binary, host, port), for storage in `L7EvalContext::allowed_secrets`
/// so the endpoint-scoped resolver built per request in
/// `scoped_context_for_request` is filtered too.
///
/// - `Some(keys)`: the exact list from OPA (`Some(vec![])` = the documented
///   "empty = no restriction" semantics; also used when no policy matched,
///   mirroring `filter_resolver_by_policy`).
/// - `None`: the OPA query failed. Consumers fail closed (no resolver).
fn query_allowed_secrets_for_policy(
    engine: &OpaEngine,
    decision: &EgressDecision,
    host: &str,
    port: u16,
    ...
    match engine.query_allowed_secrets(&input) {
        ...
        Err(e) => {
            let event = NetworkActivityBuilder::new(openshell_ocsf::ctx::ctx())
                ...
                .dst_endpoint(Endpoint::from_domain(host, port))
                .message(format!(
                    "Failed to query allowed_secrets (endpoint-scoped credentials fail closed): {e}"
                ))
                .build();
            ocsf_emit!(event);
            None
    ...
    U: TokioAsyncRead + TokioAsyncWrite + Unpin,
{
    ...
    crate::l7::rest::relay_http_request_with_options_guarded(
        ...
        crate::l7::rest::RelayRequestOptions {
            ...
            host: options.host,
            port: options.port,
            // This request's bytes were already rewritten by the caller
            // (function name: relay_rewritten_forward_request) before it
            // reached here -- any cred_inject strip-and-replace already ran
            // upstream of this call. `ForwardRelayOptions` carries no
            // cred_inject field, so there is nothing to thread through.
            cred_inject: None,
        },
    )
    .await
...
async fn handle_forward_proxy(
    ...
    // http_context() does not know about the fork's cred_inject/echo/trust
    // fields; populate them here, mirroring the CONNECT path above. Never let
    // a forwarded request reach the L7 proxy with an unfiltered resolver --
    // filter_resolver_by_policy fails closed when the calling binary isn't in
    // `allowed_secrets`.
    l7_ctx.secret_resolver =
        filter_resolver_by_policy(&opa_engine, &decision, &host_lc, port, &secret_resolver);
    l7_ctx.allowed_secrets =
        query_allowed_secrets_for_policy(&opa_engine, &decision, &host_lc, port);
    l7_ctx.cred_inject = query_cred_inject_config(&opa_engine, &decision, &host_lc, port);
    l7_ctx.echo = decision
        .endpoint
        .l7_route
        .as_ref()
        .is_some_and(|r| r.configs.iter().any(|s| s.config.echo));
    l7_ctx.trust_cache = Some(trust_cache.clone());
    l7_ctx.trust_check = query_trust_check_config(&opa_engine, &decision, &host_lc, port);
