    ...
    pub fn filtered(&self, allowed_keys: &[String]) -> Self {
        ...
        Self {
            by_placeholder: self
                ...
                .collect(),
            // Carried forward unfiltered: these are keyed by env-key name and
            // only ever make resolution *stricter* (endpoint-mismatch denial,
            // identity-bound requirement, revision-fallback allowlist) for a
            // key that's still present in `by_placeholder`. An entry here for
            // a key that got filtered out above is inert -- there is no
            // by_placeholder value left for it to apply to. Dropping these
            // instead of cloning them would fail OPEN (a filtered/per-binary
            // resolver silently losing the deny/identity-binding constraints
            // that applied to its remaining keys), so they must survive
            // `filtered()` intact.
            denied_env_keys: self.denied_env_keys.clone(),
            identity_bound_env_keys: self.identity_bound_env_keys.clone(),
            revision_fallback_allowed_revisions: self.revision_fallback_allowed_revisions.clone(),
        }
    }
...
mod tests {
    ...
    #[test]
    fn filtered_resolver_preserves_endpoint_denial_and_identity_fields() {
        // Fork fix: `filtered()` used to rebuild `Self` with only
        // `by_placeholder`, silently dropping the endpoint-mismatch denial,
        // identity-binding, and revision-fallback state (fail-open for every
        // per-binary-scoped resolver). All three must survive filtering.
        let env = HashMap::from([
            ("KEY_A".into(), "secret-a".into()),
            ("KEY_B".into(), "secret-b".into()),
            ("CFG".into(), "not-bound".into()),
        ]);
        let (_, resolver) = SecretResolver::from_provider_env(env);
        let resolver = resolver.unwrap();
        let bound: HashSet<String> = ["KEY_A".to_string(), "KEY_B".to_string()].into();
        let allowed_bound: HashSet<String> = ["KEY_A".to_string()].into();
        let fallback: HashMap<String, Arc<HashSet<u64>>> =
            HashMap::from([("KEY_A".to_string(), Arc::new(HashSet::from([3u64])))]);
        let scoped = resolver.scoped_to_env_keys(&bound, &allowed_bound, fallback);
        assert!(scoped.denied_env_keys.contains("KEY_B"));

        let filtered = scoped.filtered(&["KEY_A".to_string(), "KEY_B".to_string()]);
        assert_eq!(filtered.denied_env_keys, scoped.denied_env_keys);
        assert_eq!(
            filtered.identity_bound_env_keys,
            scoped.identity_bound_env_keys
        );
        assert_eq!(
            filtered
                .revision_fallback_allowed_revisions
        ...
        // The denied key still surfaces as a typed endpoint mismatch, not as
        // "credential unavailable".
        let err = filtered
            .resolve_placeholder_checked(&placeholder_for_env_key("KEY_B"), "header")
            .expect_err("KEY_B must stay endpoint-denied after filtering");
        assert!(err.is_endpoint_mismatch());
        // KEY_A (allowed, identity-bound) survives the filter; CFG does not.
        assert!(
            filtered
                .by_placeholder
                .contains_key(&placeholder_for_env_key("KEY_A"))
        );
        assert!(
            !filtered
                .by_placeholder
                .contains_key(&placeholder_for_env_key("CFG"))
        );
    }
