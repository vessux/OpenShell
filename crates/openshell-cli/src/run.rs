...
use openshell_core::proto::{
    ...
    ServiceStatus, SetInferenceRouteRequest, SettingScope, StartSandboxRequest, StopSandboxRequest,
...
fn add_volume_mounts_to_driver_config(
    ...
    Ok(Some(
        openshell_core::proto_struct::json_object_to_struct(root)
            .into_diagnostic()
            .wrap_err("--volume produced a driver-config value that cannot be represented")?,
    ))
}
...
pub struct SandboxCreateConfig<'a> {
    ...
    pub output: &'a str,
    /// User-supplied `--volume HOST:CONTAINER[:ro]` bind mounts, folded into
    /// the template's `driver_config` alongside any `--driver-config-json`
    /// mounts.
    pub volumes: &'a [BindVolumeSpec],
...
mod tests {
    use super::{
        ...
        add_volume_mounts_to_driver_config, build_sandbox_resource_limits,
        ...
        gateway_auth_label, gateway_authentication_state, gateway_env_override_warning,
        gateway_info_to_json, gateway_remote_label, gateway_select_with, gateway_to_json,
